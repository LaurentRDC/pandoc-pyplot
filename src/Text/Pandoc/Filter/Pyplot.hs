...
data FigureSpec = FigureSpec
    ...
    , includePath :: Maybe FilePath  -- ^ Path to a Python script to be included before the script.
...
renderScript spec = do
    includeScript <- fromMaybe (return "") $ T.readFile <$> (includePath spec)
    return $ mconcat [ "# Source code for ", T.pack $ path spec, "\n"
                     , "# Generated by pandoc-pyplot ", versionText, "\n\n"
                     , includeScript, "\n", script spec]
    where
...
parseFigureSpec (CodeBlock (id', cls, attrs) content) 
    | "pyplot" `elem` cls = fmap Just $ createInclusion $ Map.lookup targetKey attrs'
    ...
    where
        ...
        createInclusion  Nothing  = do
            -- In case of a figure with no stated directory or target, the directory is called "generated"
            let dir = Map.findWithDefault "generated" directoryKey attrs'
        ...
        createInclusionWithExplicitTarget fname = FigureSpec { 
            ...
            where
                dir = Map.findWithDefault "." directoryKey attrs'
