...
parseFigureSpec :: Maybe Format -> Block -> PyplotM (Maybe FigureSpec)
parseFigureSpec mfmt (CodeBlock (id', cls, attrs) content)
  ...
  where
    ...
    figureSpec lib = do
        ...
        -- In case of output format that is HTML, we need to make sure that the conditions are satisfied:
        -- * Target format is also HTML
        -- * Rendering library is Plotly
