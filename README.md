...
For cleaner output (e.g. PDF), you can turn this off via the `with-links=false` key:
...
To avoid repetition, `pandoc-pyplot` can be configured using simple YAML files. `pandoc-pyplot` will look for a `.pandoc-pyplot.yml` file in the current working directory:
